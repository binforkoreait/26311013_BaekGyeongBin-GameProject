...
class Map
{
  public:
    ...
    {
        int left = int(std::floor((p.x - radius) / Tile));
        int right = int(std::floor((p.x + radius) / Tile));
        int top = int(std::floor((p.y - radius) / Tile));
        int bottom = int(std::floor((p.y + radius) / Tile));
        for (int y = top; y <= bottom; ++y)
        {
            for (int x = left; x <= right; ++x)
            {
                if (x < 0 || y < 0 || x >= Columns || y >= Rows || walls[y][x])
                {
                    return true;
                }
            }
    ...
    bool Visible(Vec a, Vec b) const
    {
        ...
        for (int i = 1; i <= steps; ++i)
        {
            ...
            if (Blocked({a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t}, 2))
            {
                return false;
            }
        }
...
class Game
{
  public:
    ...
    void LoadStage()
    {
        ...
        items.clear();
        banner = 2.5f;
        const Vec spawns[] = {{220, 180}, {460, 140}, {540, 420}, {740, 180}, {820, 460},
                              {380, 460}, {700, 380}, {860, 100}, {180, 420}};
        for (int i = 0; i < 3 + stage * 2; ++i)
        {
            enemies.push_back({spawns[i], 60, 0});
        }
    ...
    void Update(float dt, Input input)
    {
        ...
        {
            bullets.push_back(
                {player.position, {player.facing.x * 440, player.facing.y * 440}, 1.4f});
            player.cooldown = 0.23f;
            sounds.push_back(Sound::Attack);
        }
        for (auto &bullet : bullets)
        {
            bullet.life -= dt;
            // 한 프레임을 나눠 움직여 탄환이 벽이나 적을 통과하지 않게 한다.
            for (int step = 0; step < 4 && bullet.life > 0; ++step)
            {
                bullet.position.x += bullet.velocity.x * dt / 4;
                bullet.position.y += bullet.velocity.y * dt / 4;
                if (map.Blocked(bullet.position, 4))
                {
                    bullet.life = 0;
                    break;
                }
                ...
                {
                    if (enemy.hp > 0 && Distance(enemy.position, bullet.position) < 19)
                    {
                        enemy.hp -= 30;
                        bullet.life = 0;
                        if (enemy.hp <= 0)
                        {
                            sounds.push_back(Sound::EnemyDeath);
                        }
                        else
                        {
        ...
        bullets.erase(std::remove_if(bullets.begin(), bullets.end(),
                                     [](const Bullet &b) { return b.life <= 0; }),
                      bullets.end());
        ...
        for (auto &enemy : enemies)
        {
            ...
            if (d < 300 && d > 25 && map.Visible(enemy.position, player.position))
            {
                ...
                map.Move(enemy.position,
                         {(player.position.x - enemy.position.x) / d * speed * dt,
                          (player.position.y - enemy.position.y) / d * speed * dt},
                         14);
            }
            if (Distance(enemy.position, player.position) < 30 && enemy.cooldown <= 0 &&
                player.immunity <= 0)
            {
                player.hp = (std::max)(0, player.hp - 15);
                player.immunity = 0.85f;
                enemy.cooldown = 1;
                sounds.push_back(Sound::Damage);
                if (player.hp == 0)
                {
                    screen = Screen::GameOver;
                    sounds.push_back(Sound::GameOver);
                    return;
                }
            }
        }
        for (auto &item : items)
        {
            if (!item.taken && Distance(player.position, item.position) < 27)
            {
                ...
                item.taken = true;
                sounds.push_back(Sound::Item);
            }
        }
        if (hasKey && Distance(player.position, map.exit) < 27)
        {
            sounds.push_back(Sound::Clear);
            if (stage == 3)
            {
                screen = Screen::Clear;
            }
